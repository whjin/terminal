			...
			<div class="face-content">
				...
				<div class="face-box">
					<div class="face-menu-list">
						<div class="face-menu-item" :class="currentPage == 1 ? 'face-menu-active' : ''"
							@click="switchPage(1)">
							<common-icons iconType="icondispose" size="38" color="#fff" />
							<text>面对面登记</text>
						</div>
						<div class="face-menu-item" :class="currentPage == 2 ? 'face-menu-active' : ''"
							@click="switchPage(2)">
					...
					<div v-if="currentPage == 1" class="face-register-container" @click.stop="hideModal">
						<div class="face-form-content">
							<div class="face-form-box">
								...
								<div class="face-select">
									<label>面对面民警：</label>
									<xfl-select :list="policeList" :initValue="policeInfo.value" clearable
										:placeholder="'请选择面对面民警'" @change="selectPolice"
										@clear="clearSelectPolice"></xfl-select>
								</div>
								<div class="face-select">
									<label>登记时间：</label>
									<e-picker mode="dateTime" class="picker-img" @change="selectRegDate"
										:placeholder="'请选择登记时间'">
										<div class="face-date">{{ regDate }}</div>
									</e-picker>
								</div>
								<div class="face-ld-select">
									<label>面对面类型：</label>
									<ld-select ref="faceTypeRef" multiple :list="faceTypeList" label-key="value"
										value-key="code" :widthStyle="716" :placeholder="'请选择面对面类型'" clearable
										v-model="selectFaceType" @change="faceTypeChange"></ld-select>
								</div>
								<div class="face-select face-remark">
									<label>情况记录：</label>
									<textarea :value="faceRemark" :maxlength="-1" @input="faceRemarkChange"
										disabled></textarea>
								</div>
							</div>
						</div>
					...
					<div v-if="currentPage == 2" class="face-record-container">
						<div class="record-form-content">
							<div class="face-form-box">
								...
								<div class="face-select">
									<label>登记时间：</label>
									<e-picker mode="date" class="picker-img" @change="selectRecordDate"
										:placeholder="'请选择登记时间'">
								...
								<div class="face-select">
									<label>面对面类型：</label>
									<xfl-select :list="faceTypeList" clearable :placeholder="'请选择面对面类型'"
										@change="faceTypeSearch" @clear="clearFaceType"></xfl-select>
								</div>
							</div>
							<div class="face-button-box">
...
export default {
	...
	methods: {
		...
		// 获取民警信息
		async getPoliceInfo() {
			const { accountName, name } = this.personInfo;
			this.policeInfo.code = accountName;
			this.policeInfo.value = name;
		...
		async getFaceTypeInfo() {
			...
			if (res.state.code == 200) {
				...
				this.faceTypeList.forEach(item => {
					if (item.isSelect == '1') {
						selectInfo.push(item.code);
					}
				});
				this.faceTypeChange(selectInfo);
			}
		},
		// 选择面对面民警
		selectPolice(item) {
		...
		// 选择面对面类型
		faceTypeChange(item) {
			this.selectFaceType = item;
			let valueList = [];
			this.faceTypeList.forEach(type => {
				this.selectFaceType.forEach(code => {
					if (type.code == code) {
						valueList.push(type.value);
					}
				});
			});
			this.faceType = this.selectFaceType.toString();
			this.faceRemark = valueList.toString();
		},
		// 清空面对面类型
		clearFaceType() {
			this.selectFaceType = [];
		...
		// 隐藏下拉框
		hideModal() {
			this.$refs.faceTypeRef && this.$refs.faceTypeRef.hideModal();
		},
		// 选择面对面类型（查询）
		faceTypeSearch(item) {
