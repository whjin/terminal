<template>
  <view class="check main-area" @click.stop="handleCloseLdSelect">
    ...
    <view class="uni-flex uni-flex-item" style="height: 85%">
      <view class="uni-flex uni-column page-menu-area">
        <view
          ...
          :class="currentPage == 1 ? 'page-menu-activate' : ''"
          @click="switchPage(1)"
        >
          <image
            src="/static/images/room/record.png"
        ...
        <view
          class="uni-flex uni-column page-menu"
          :class="currentPage == 2 ? 'page-menu-activate' : ''"
          @click="switchPage(2)"
        >
          <image
            src="/static/images/room/record.png"
      ...
      <view
        v-if="currentPage == 1"
        class="uni-flex uni-column record-registration"
        style="width: 945upx"
      >
        <scroll-view
          scroll-y="true"
        ...
        >
          <view class="basic-content">
            ...
            >
              ...
              <input
                v-else
                class="value"
                :class="{ 'had-border': item.inputType == 'input' }"
                :type="item.value == 'participantsNum' ? 'number' : 'text'"
                v-model="saveCheckParams[item.value]"
                :disabled="item.inputType == 'text'"
          ...
          <view class="primary-content">
            ...
            <ld-select
              ref="ldSelect"
              :multiple="true"
              :list="checkContentList"
              label-key="value"
              value-key="code"
              :placeholder="
                checkContentList.length ? '请选择检查内容' : '请先选择检查类型'
              "
              :widthStyle="721"
              clearable
              v-model="checkContentValue"
              :disabled="checkContentList.length ? false : true"
              @change="setCheckContent"
            ></ld-select>
          </view>
          <view class="primary-content">
            ...
            <input
              class="value"
              v-model="saveCheckParams.checkContraband"
              type="text"
              @input="handleInputChange"
            />
          </view>
          ...
          <view class="primary-content">
            ...
            <input
              class="value"
              v-model="saveCheckParams.remarks"
              type="text"
              @input="handleInputChange"
            />
          </view>
        ...
        <view class="btn-box" style="margin-left: 800upx; margin-bottom: 10upx">
          ...
          <view
            @click.stop="cancel"
            class="page-button-empty"
            style="margin-right: 14.58upx"
            >取消登记</view
          >
        </view>
      </view>
      <view
        ...
        style="width: 1039.58upx; padding-left: 20upx"
      >
        <view class="search-area">
          <view class="search-params">
            <view class="search-params-item">
              <view class="params-title" style="margin-right: 16.7upx"
                >监室号：</view
              >
              <input
                class="value"
                type="text"
                v-model="saveCheckParams.roomName"
                disabled
              />
            </view>
            <view class="search-params-item">
        ...
        >
          <view
            ...
            :key="index"
          >
            <view class="uni-flex-item table-td">{{
              saveCheckParams.roomName
            }}</view>
            ...
            <view class="uni-flex-item table-td">{{
              item.checkContentValue
            }}</view>
            <view class="uni-flex-item table-td">{{
              item.checkContraband
            }}</view>
            <view class="uni-flex-item table-td"
              ><a class="check-btn" @click.stop="checkDetail(item)"
                >查看</a
              ></view
            >
          </view>
    ...
    <div class="neil-modal-container">
      <neil-modal
        :show="isShowCheckDetail"
        :autoClose="true"
        @close="handleCloseCheckDetail"
      >
        ...
        <view
          class="page-horizontal-divider"
          style="margin-bottom: 28upx"
        ></view>
        <scroll-view class="check-detail" scroll-y>
          <view class="basic-content">
            ...
            >
              ...
              <input
                v-if="
                  item.value == 'roomName' || item.value == 'registerPolice'
                "
                class="value had-border"
                v-model="saveCheckParams[item.value]"
                disabled
              />
              <input
                v-else
                class="value had-border"
                v-model="selectRecord[item.value]"
                disabled
              />
            </view>
          </view>
          <view class="primary-content violate-goods">
            ...
            <scroll-view class="value" style="height: 72.4upx" scroll-y="true">
              <textarea
                :value="selectRecord.checkContentValue"
                disabled
                auto-height
                :maxlength="-1"
          ...
          <view class="primary-content">
            ...
            <input
              class="value"
              v-model="selectRecord.checkContraband"
              disabled
            />
          </view>
          <view class="primary-content violate-goods">
      ...
      >
        ...
        <image
          class="preview-img"
          :src="previewImgUrl"
          @click.stop="handleClosePreviewModal"
        ></image>
      </neil-modal>
      <!-- 拍照弹框 -->
      <neil-modal
