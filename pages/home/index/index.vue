...
import { isNullStr, dateFormat, str2Array } from "@/common/utils/util.js";
...
export default {
  ...
  onLoad() {
    ...
    plus.globalEvent.addEventListener("exceptionListener", function (e) {
      ...
      let eventContent = "";
      eventContent = `${uni.getStorageSync("terminalInfo").terminalCode
  ...
  methods: {
    ...
    async getPoliceInfo() {
      ...
      let res = await Api.apiCall("get", Api.index.getPoliceInfo, { roomId }, true);
    ...
    async getDynamicInfo(state) {
      let { terminalCode } = uni.getStorageSync("terminalInfo");
      let res = await Api.apiCall(
        "get",
    ...
    // 重置倒计时长
    initCountTimeout() {
      if ([11, 17, 19].includes(this.currentTab)) {
    ...
    handleHomeModal(item) {
      let flag = (["play", "pause"].includes(this.educationVideoInfo.status) &&
        item.index === 18) || item.url === "OnlineCall";
      if (flag) {
        return;
      }
      if (!this.intercomState) {
    ...
    handleRestartDev() {
      uni.closeSocket();
      getApp().globalData.Utils.reboot();
    },
    // 应用重启弹框
    restartAppModel() {
          ...
          (d, status) => {
            ...
            if (status == 200) {
              ...
              getApp().globalData.UpdateApp.install(d.filename, (res) => {
                Log.writeLog("getApp().globalData.UpdateApp.install()】", false);
                if (res.state) {
                  console.log("APP安装成功");
                } else {
                  console.log("APP安装失败");
    ...
    // 回声校准
    echoCalibrate() {
      getApp().globalData.Sip.startEchoCancellerCalibration();
    ...
    async faceRecognition(base64Str = "") {
      ...
      let res = await Api.apiCall(
        "post",
        Api.prisoner.prisonerImgOneToMany,
        params,
        true,
        true
      );
      ...
      this.socketTask.onMessage((res) => {
        ...
        if (info.type == this.$config.controlType.HEARTBEAT) {
          ...
          if (info.msg == "0") {
            ...
            console.log("设备校时");
            getApp().globalData.Utils.setTime(info.extend);
          } else if (info.msg == "3") {
            // 关闭视频
            clearInterval(this.videoTimer);
            this.stopVideo();
            getApp().globalData.Sip.logout();
            // 设备重启
            uni.closeSocket();
            getApp().globalData.Utils.reboot();
          } else if (info.msg == "4") {
    ...
    webSocketReConnct() {
      ...
      this.socketTimer = setTimeout(() => {
        console.log(
          ...
          uni.getStorageSync("terminalInfo").terminalCode
        );
        this.connectWebSocketInit(uni.getStorageSync("terminalInfo").terminalCode);
      }, this.websocketTime);
    },
    ...
    async getLoginPersonInfo(data) {
      ...
      let res = await Api.apiCall("get", url, { roomNo, mKey });
      if (!this.showRecognitionDialogs) return;
      if (res.state.code == 200) {
        this.closeModal("RecognitionDialogs");
        let result = Object.assign(res.data, data);
        this.loginStatusHanlder(result);
      }
    },
    // 设置人脸认证登录人员信息
    ...
    // 获取应急报警信息
    async saveAlarmInfo() {
      let { terminalId } = uni.getStorageSync("terminalInfo");
      let params = {
        alarmEvents: "1",
        alarmLevel: "1",
        status: "n",
      ...
      if (res.state.code == 200) {
        let { terminalCode, roomName } = uni.getStorageSync("terminalInfo");
    ...
    async setDynamicInfo(state) {
      ...
      let params = {
        controlId,
        type: "402",
        content: `${state}${terminalName}电教视频`,
