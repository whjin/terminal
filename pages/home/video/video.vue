<template>
	<div class="video-container" @touchstart.stop="initCountTimer">
		<div class="video-wrapper">
			<div class="video-content">
				<div class="video-left">
                  ...
                  videoId == item.videoId ? 'video-select-img' : 'video-img'
                " v-for="(item, index) in videoList" :key="index" @click="selectVideoChange(item)">
					...
					<div v-if="isPlay" class="video-control">
						<video id="myVideo" class="my-video" :src="videoUrl" codec="software" object-fit="fill" autoplay @ended="videoPlayEnded" @error="videoPlayError"></video>
					</div>
				</div>
				...
				<div class="video-right">
					...
					<div class="video-right-btn" v-if="!videoInfo.status">
	...
	methods: {
			...
			if (res.state.code == "200") {
				...
				if (result.length) {
					result.map((item) => {
						...
						item.imgUrl = this.fileUrlPrefix + item.imgUrl;
						item.videoUrl = this.fileUrlPrefix + item.videoUrl;
					});
				}
		...
		async setDynamicInfo (state) {
			let params = {
				...
				type: "401",
				content: `${state}点播${uni.getStorageSync("terminalInfo").name}视频`,
		...
		async getVideoPlayInfo () {
			...
			let roomNo = uni.getStorageSync("terminalInfo").roomNo;
			let res = await Api.apiCall("get", Api.index.getByVideoPlayTime + `?vId=${vId}&roomNo=${roomNo}`, null);
			if (res.state.code == "200") {
		...
		videoPlayError (e) {
			this.stopPlayVideo(true);
			this.$parent.handleShowToast("视频播放错误，请联系管教！", "bottom", 10000);
		},
	},
};
</script>
