    ...
    <view class="main-area">
      ...
      <view class="uni-flex uni-flex-item">
        <view class="uni-flex uni-column page-menu-area">
          <view v-for="item in tabConfig" :key="item.key" class="uni-flex uni-column page-menu"
            :class="{ 'page-menu-activate': currentGoodType === item.type }" @click="switchPage(item.type)">
        ...
        <view class="uni-flex uni-column uni-flex-item">
          <scroll-view scroll-y="true" class="uni-flex uni-row"
            style="width: 1140upx; flex-direction: row; height: 500upx;" @scrolltolower="handleScrolltolower">
            <view class="uni-flex" style="flex-wrap: wrap;">
              <view class="symptoms" v-for="item in goodsList" :key="item.id">
                <view class="uni-flex" style="margin-top: 20upx; margin-left: 25.69upx; float: left;">
                  <image :src="fileUrlPrefix + item.picUrl" style="width: 96.52upx; height: 102.75upx;"></image>
                </view>
                <scroll-view class="uni-flex text-describe text-describe-name" scroll-x>
                  名称：{{ item.goodName }}</scroll-view>
                <scroll-view class="uni-flex text-describe" scroll-x>状态：{{ item.statusName }}</scroll-view>
                <view class="uni-flex button button-repair"
                  :class="{ 'button-disable-repair': item.operateStatus === 0 && (item.status === 0 || item.status === 1 || item.status === 2 || item.status === 3) }"
                ...
                <view class="uni-flex button button-repair"
                  :class="{ 'button-disable-repair': !item.operateStatus && item.status !== 3 }"
      ...
      <neil-modal :show="isShowConfirmModal" @close="closeConfirmModal">
        <view class="uni-flex uni-column" style="width: 500upx; height: 400upx;">
          <view class="uni-flex uni-flex-item uni-column" style="justify-content: center; align-items: center;">
            <text style="font-size: 25upx; font-family: 400;">{{ isConfirmRecord ? '是否确认本次处理结果' : `确认提交物品${typeName}`
            }}？</text>
          </view>
