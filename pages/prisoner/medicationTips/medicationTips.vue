<template>
  <view class="medication-tips" @touchstart.stop="initCountTimer">
    <view class="main-area">
      <view class="uni-flex page-title header">
        ...
        <view class="btn-box">
          ...
          <view v-if="!isTheMedicine && (curPage == 1)" class="btn-item blue-btn" :class="{ 'disabled': !medicationPlanList.length }" @click="hanleMedication('start')">开始服药</view>
        </view>
      </view>
      <view class="content">
        <view class="tips" v-if="isTheMedicine">
          <view class="left-box inner-glow-box">
            ...
            <view class="video-box">
              ...
              <video :src="videoUrl" codec="software" autoplay muted :controls="false" style="width:678.473upx; height:359.73upx;" object-fit="fill">
              </video>
            </view>
          </view>
          <view class="right-box inner-glow-box">
            ...
            <view class="steps-box">
              <view class="steps-item" :class="{'next-steps': item.hasNextSteps}" v-for="item in stepsDetails" :key="item.imgUrl">
        ...
        <template v-else>
          <div class="medication-menu-list" v-if="debounceSwitchPage">
            <div v-for="item in medicationMenuList" :key="item.page" class="medication-menu-item" :class="{ 'medication-menu-active': curPage == item.page }" @click="debounceSwitchPage(item.page)">
              <common-icons :iconType="item.iconStyle.iconType" :size="item.iconStyle.size" :color="item.iconStyle.color" />
          ...
          <div class="medication-table-container">
            <div class="table-box">
              <div class="table-head">
                <div class="medication-head-item" v-for="item in medicationTableList" :key="item.code">{{ item.title }}</div>
              </div>
              <scroll-view scroll-y="true" class="medication-table-scroll" @scrolltolower="handleRecorListScrolltolower">
                <div class="medication-table-content" v-for="(item, index) in medicationColumns" :key="index">
                  <div class="medication-table-item" v-for="i in medicationTableList" :key="i.code" :style="i.code == 'status' && curPage == 1 ? 'color:#D22D2D;' : ''" @click="handleClickTableItem(i.code)">{{ item[i.code] }}</div>
                </div>
              </scroll-view>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    hanleMedication (type) {
      if (type == 'start') {
        this.handleRecord('start', () => {
          ...
          let url = uni.getStorageSync('rtspUrl') + '@' + uni.getStorageSync('terminalInfo').embeddedIp + '/stream0';
    ...
    async getMedicationList (type) {
      let api = type == 1 ? Api.prisoner.medicationTips.medicationPlanList : Api.prisoner.medicationTips.medicationRecordList;
      let params = type == 1 ? {
      ...
      if (res.state.code == "200") {
        ...
        if (type == 1) {
          ...
          if (this.medicationRecordPageParam.pageIndex == 1) {
            ...
            this.medicationRecordList = this.medicationRecordList.concat(recordList);
          }
    ...
    async handleRecord (type, cb) {
      ...
      let api = type == 'start' ? Api.prisoner.medicationTips.startRecord : Api.prisoner.medicationTips.endRecord;
      let url = uni.getStorageSync('rtspUrl') + '@' + uni.getStorageSync('terminalInfo').embeddedIp + '/stream0';
      let params = {
        data: {
          rtspUrl: url
    ...
    async saveMedicineRecord (status, url) {
      ...
      let res = await Api.apiCall("post", Api.prisoner.medicationTips.saveMedicineRecord, {
        data: params
      }, true);
      this.initCountTimer();
    }
