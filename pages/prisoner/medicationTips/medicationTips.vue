          ...
          <view class="left-box inner-glow-box">
            ...
            <view class="video-box">
              ...
              <video :src="videoUrl" codec="software" autoplay muted :controls="false"
                style="width:678.473upx; height:359.73upx;" object-fit="fill">
              </video>
            </view>
          </view>
          ...
          <div class="medication-table-container">
            <div class="table-box">
              <div class="table-head">
                <div class="medication-head-item" v-for="item in medicationTableList" :key="item.code">{{ item.title }}
                </div>
              </div>
              <scroll-view scroll-y="true" class="medication-table-scroll" @scrolltolower="handleRecorListScrolltolower">
                <div class="medication-table-content" v-for="(item, index) in medicationColumns" :key="index">
                  <div class="medication-table-item" v-for="i in medicationTableList" :key="i.code"
                    :style="i.code === 'status' && curPage === 1 ? 'color:#D22D2D;' : ''"
                    @click="handleClickTableItem(i.code)">{{ item[i.code] }}</div>
                </div>
              </scroll-view>
            </div>
          </div>
...
export default {
  ...
  beforeDestroy() {
    this.videoUrl && this.handleRecord("end");
  },
  methods: {
    // 重置倒计时
    initCountTimer() {
    ...
    // 获取服药签名状态
    async getSignatureState(cb) {
      const { id } = this.medicationPlanList[0];
      let params = {
        data: {
          prescribeId: id,
          rybh: this.personInfo.rybh,
      ...
      let res = await Api.apiCall("post", Api.prisoner.medicationTips.findMedicineFinish, params);
      if (res.state.code == 200) {
        cb & cb(res);
      }
    },
    ...
    hanleMedication(type) {
      if (type === 'start') {
        // 获取服药签名状态
        this.getSignatureState(res => {
          if (Reflect.has(res, "data")) {
            this.medicineRecordId = res.data;
        ...
        this.handleRecord('end', (url) => {
          ...
          this.saveMedicineInfo(status, url).then(() => {
            this.showSignModal = true;
          });
        });
      }
    ...
    signSuccess(res) {
      this.handlePathToBase64(res.tempFilePath).then(base64Str => {
        this.saveMedicineInfo(1, '', base64Str);
    ...
    async handleRecord(type, cb) {
      ...
      this.videoUrl = uni.getStorageSync('rtspUrl') + '@' + uni.getStorageSync('terminalInfo').embeddedIp + '/stream0';
      let params = {
        data: {
    ...
    // 保存服药记录
    async saveMedicineInfo(status, url, signature = '') {
      let params = {
        prescribeId: '',
        doctorName: '',
        doctorProof: '',
      ...
      if (res.state.code == 200) {
        this.medicineRecordId = res.data;
        this.$parent.handleShowToast("保存服药信息成功");
      }
    },
    // 图片路径转Base64
