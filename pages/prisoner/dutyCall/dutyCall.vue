<template>
  <div class="rotation-container" @touchstart.stop="initCountTimer">
      ...
      <div class="rotation-content">
        ...
        <div class="rotation-box">
          <div class="rotation-menu-list">
            <div class="rotation-menu-item" :class="currentPage == 1 ? 'rotation-menu-active' : ''" @click="switchPage(1)">
              <common-icons iconType="iconweekdate" size="38" color="#fff" />
              <text>一周值班表</text>
            </div>
            <div class="rotation-menu-item" :class="currentPage == 2 ? 'rotation-menu-active' : ''" @click="switchPage(2)">
              <common-icons iconType="iconregister" size="42" color="#fff" />
              <text>监室值班登记</text>
            </div>
            <div class="rotation-menu-item" :class="currentPage == 3 ? 'rotation-menu-active' : ''" @click="switchPage(3)">
            ...
            <div class="table-box">
              ...
              <scroll-view scroll-y="true" class="rotation-table-scroll">
                <div class="table-content" v-for="item in dutyWeekPlanList" :key="item.timeQuantum">
                  ...
                  <div class="table-content-item" style="flex: 1" v-for="(person, index) in item.dutyList" :key="index" :class="{ 'current-date': person.highlight }">
                    {{ person.names }}
                  </div>
                </div>
              </scroll-view>
          ...
          <div v-if="currentPage == 2" class="rotation-register-container">
            ...
            <div class="register-select">
              <label>值班情况：</label>
              <xfl-select :list="dutyStatusList" :clearable="false" :placeholder="'请选择值班情况'" @change="dutyStatusChange"></xfl-select>
            </div>
          ...
          <div v-if="currentPage == 3" class="rotation-record-container">
            <div class="record-form-box">
              <div class="record-form-left">
                ...
                <div class="form-duty-schedule">
                  ...
                  <xfl-select :list="timeQuantumList" :clearable="false" :placeholder="'请选择班次'" @change="handleShiftChange"></xfl-select>
                </div>
              </div>
            ...
            <div class="record-table-box">
              <div class="table-head">
                <div class="table-head-item" v-for="(item, index) in recordColumns" :key="index" :style="{ flex: item.flex }">
                  {{ item.title }}
                </div>
              </div>
    ...
    <recognition-dialogs ref="recognitionDialogs" useFor="rotation" :callConfig="callConfig" :isShow="showRecognitionDialogs" @close="recognitionDialogsClose" @fingerRecognitionSuccess="fingerRecognitionSuccess" @faceRecognitionSuccess="faceRecognitionSuccess"></recognition-dialogs>
  </div>
</template>
      ...
      if (this.dutyList.length) {
        ...
        this.$nextTick(() => {
          this.$refs.recognitionDialogs &&
            this.$refs.recognitionDialogs.startRecognition(9);
        });
      } else {
