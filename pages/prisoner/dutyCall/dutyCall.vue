  ...
  methods: {
    ...
    startDutyCall() {
      if (this.dutyList.length) {
        ...
        currentPages().handleShowToast("当前没有值班任务");
      }
    },
    // 指纹认证成功回调
    ...
    async saveDutySignRecord(item) {
      ...
      if (res.state.code == 200) {
        currentPages().voiceBroadcast(`${item.name}签到成功`);
        setTimeout(() => {
          this.recognitionDialogsClose();
        }, 2000);
      } else {
        this.showRecognitionDialogs = false;
        currentPages().voiceBroadcast(res.state.msg || "签到失败");
      }
    },
    // 提交保存值班登记
    handleRegSubmit() {
      ...
      if (!hadAuth) {
        currentPages().handleShowToast("没有提交权限");
        return;
      }
      if (!this.dutyStatus) {
        currentPages().handleShowToast("请选择值班情况");
        return;
      }
    ...
    async updateSituation() {
      ...
      if (res.state.code == 200) {
        currentPages().handleShowToast("提交成功");
      }
    },
    // 取消值班登记
    handleRegCancel() {
    ...
    async signRecordSearch() {
      ...
      if (res.state.code == 200) {
        if (this.dutyRecordPageParam.pageIndex == 1 && !res.data.length) {
          currentPages().handleShowToast("暂无更多数据", "center");
        }
