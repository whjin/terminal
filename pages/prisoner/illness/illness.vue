						...
						<scroll-view scroll-y="true" class="illness-reserve-scroll">
							<div class="illness-symptom" v-for="(list, index) in symptomList" :key="index">
								...
								<div class="illness-symptom-list">
									<div class="symptom-item mood-img"
										:class="{ 'mood-select-img': item.value == commonSymptom || item.value == emergencyDisease || item.value == otherSymptom }"
							...
							<div class="illness-symptom">
								...
								<div class="illness-bottom">
									...
									<div v-if="debounceMeasure" class="btn-submit" @click="debounceMeasure">重测</div>
								</div>
							</div>
							<div class="illness-symptom">
								<div class="title">
						...
						<div class="record-table-box">
							<div class="table-head">
								<div class="table-head-item" v-for="(item, index) in illnessColumns" :key="index"
									:style="{ flex: item.flex }">{{ item.title }}</div>
							</div>
							<scroll-view scroll-y="true" class="record-table-scroll" @scrolltolower="handleScrolltolower">
								<div class="table-content" v-for="(item, index) in recordList" :key="index">
									...
									<div class="record-table-item" style="flex:1">
										<div class="btn-query"
											:class="{ disabled: !((nowTimestamp > item.date + 3600000) && item.statusCode == 1) }"
											v-if="debounceUrge" @click="debounceUrge(item)">催办</div>
									</div>
								</div>
							</scroll-view>
						</div>
